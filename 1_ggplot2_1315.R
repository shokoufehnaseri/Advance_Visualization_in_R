...
ggplot() + 
  geom_point(data = boston, aes(y = MEDV, x = RM)) +
  geom_smooth(data = boston[boston$RM > 4, ], aes(y = MEDV, x = RM))
...
ggplot(movies, aes(x=budget, y=votes)) +
  geom_point() +
  geom_smooth() +
...
ggplot() + geom_point(data = boston,  aes(y = MEDV, x = RM, color = "Something"))  
...
# a possibility to define your own shapes, but for now we will focus only on the 
...
ggsave(filename = 'Plot_1.png', # file name
       plot = last_plot(), # object to export
...
# Hint: There isn't one correct answer in this exercise. You may use different
...
movies <- movies %>% 
  mutate(
    genre = case_when(Romance==1 & Comedy==1 ~ 'Rom. Com.',
                      Romance==1 ~ 'Romance',
                      Comedy==1 ~ 'Comedy',
                      T~'Other')
  )

ggplot(data=movies, aes(x=year, y=length, color=genre)) +
  geom_point(alpha=.4) +
  geom_smooth(se=F)
